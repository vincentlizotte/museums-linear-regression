...
def import_data():
    ...
    # These are the delimiters that allow location the population value in the editor's text box
                ...
                if city_from_csv is not None:
                    ...
                    for delimiter_start, delimiter_end in wikipedia_edit_population_delimiters:
                        if delimiter_start in editor_box_text:
                            try:
                                population_split = editor_box_text.split(delimiter_start)[1].split('=')[1].split(delimiter_end)[0].strip().replace(',', '')
                                population = int(population_split)
                                break
                            except:
                                pass
